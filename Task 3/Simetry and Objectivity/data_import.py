...
def function(random, G7):
    ...
    #P = np.concatenate((P6,P2), axis = 0)
    SE = np.concatenate((SE6,SE0,SE1,SE2,SE7), axis = 0)
    ERROR = np.concatenate((ERROR6,ERROR0,ERROR1,ERROR2,ERROR7), axis = 0)
    
    F_rot1 =[]
    F2 = F
    for k in range(0,len(random)):
        F_rot1.append(random[k,:,:]@F)
        
    for l in range (0, len(F_rot1)):
        F2 = np.concatenate((F2,F_rot1[l]),axis=0)
        
    F_rot =[]
    for i in range(1,24):
        F_rot.append(F@G7[i,:,:])
        
    for j in range (0, len(F_rot)):
        F = np.concatenate((F,F_rot[j]),axis=0)
        
    F = np.concatenate((F2,F),axis=0)
    
    P_rot1 =[]
    P2 = P
    for k in range(0,len(random)):
        P_rot1.append(random[k,:,:]@P2)
        
    for l in range (0, len(P_rot1)):
        P2 = np.concatenate((P2,P_rot1[l]),axis=0)
        
    P_rot =[]
    for i in range(1,24):
        P_rot.append(P@G7[i,:,:])
        
    for j in range (0, len(P_rot)):
        P = np.concatenate((P,P_rot[j]),axis=0)
        
    P = np.concatenate((P2,P),axis=0)
